...
var rabbitmqConf = flag.String("f", "conf/rabbitmq.conf", "configuration file to load")
var logPath = flag.String("l", "log/rabbitmq", "configuration file to log")
var rabbitmq = &RabbitMQ{}

func init() {
	flag.Parse()
	utils.RouteLog(*logPath)
	if err := conf.LoadEx(rabbitmq, *rabbitmqConf); err != nil {
		log.Fatal("config.Load failed:", err)
	}
}

func main() {
	log.Info("start collect rabbitmq metric data")
...
type RabbitMQ struct {
	...
	ResponseHeaderTimeout common.Duration `json:"header_timeout"`
	ClientTimeout         common.Duration `json:"client_timeout"`

	Nodes     []string
	Queues    []string
	Exchanges []string

	QueueInclude []string `json:"queue_name_include"`
	QueueExclude []string `json:"queue_name_exclude"`
...
func (r *RabbitMQ) Gather(acc telegraf.Accumulator) error {
	if r.Client == nil {
		tlsCfg, err := common.GetTLSConfig(
