...
type Mysql struct {
	...
	GatherSubordinateStatus                   bool     `json:"gather_subordinate_status"`
...
func (m *Mysql) gatherServer(serv string, acc telegraf.Accumulator) error {
	...
	if len(m.IntervalSlow) > 0 {
		if uint32(time.Since(lastT).Seconds()) >= scanIntervalSlow {
			...
			if err != nil {
				log.Debugf("gatherGlobalVariables error:", err)
			} else {
				log.Debugf("gatherGlobalVariables success")
			}
			lastT = time.Now()
		}
	}
	...
	if m.GatherSubordinateStatus {
		...
		if err != nil {
			//return err
			log.Debugf("gatherSubordinateStatus error: ", err)
		} else {
			log.Debugf("gatherSubordinateStatus success")
		}
	}
